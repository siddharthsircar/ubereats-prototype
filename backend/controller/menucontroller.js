...
const addItem = async (menu_id, item_image, item_name, item_desc, item_type, item_price) => {
    try {
        const itemObject = await menu.create({ menu_id, item_image, item_name, item_desc, item_type, item_price });
        return {
            statusCode: 201,
            body: itemObject,
        };
...
const updateItem = async (item_id, updateData) => {
    try {
        const updateObject = await menu.update(
            updateData,
            {
                where: {
                    item_id: item_id
