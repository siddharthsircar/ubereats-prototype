...
const userRegisterDispatcher = (payload) => ({
    type: USER_REGISTER, payload,
});

const restLoginDispatcher = (payload) => ({
    type: RESTAURANT_LOGIN, payload,
});

const restRegisterDispatcher = (payload) => ({
    type: RESTAURANT_REGISTER, payload,
});
...
const loginRest = (payload) => (dispatch) => {
    ...
    axios.post('http://localhost:7000/restaurant/login', payload)
        .then((res) => {
            if (res.status === 200) {
                console.log(res);
...
const registerRest = (payload) => (dispatch) => {
    console.log("Restaurant Payload: ", payload);
    axios.post('http://localhost:7000/restaurant/register', payload)
        .then((res) => {
            if (res.status === 201) {
                console.log(res);
...
export {
    userLoginDispatcher, logoutDispatcher, userRegisterDispatcher, restRegisterDispatcher, restLoginDispatcher, loginUser, registerUser, loginRest, registerRest
};